    ...
    try:
        ...
        for page in doc:
            ...
            if header_idx is None:
                # No table header detected on this page
                continue

            # Determine column x positions from header keywords (approximate)
            ...
            for i in range(header_idx + 1, len(ordered_lines)):
                ...
                if not desc:
                    # If description empty, try all non-numeric words not in other columns
                    pass
                # Parse code
...
def extract_from_bytes(file_bytes, filename: str = '') -> dict:
    ...
    try:
        ...
        # Prefer coordinate-based items when available (more accurate)
        coord_items = _extract_items_with_coordinates(file_bytes)

        # Format items with all extracted fields
        base_items = coord_items if coord_items else parsed.get('items', [])
        items = []
...
def build_invoice_json(parsed: dict) -> dict:
    ...
    totals = {
        'sub_total': float(parsed.get('subtotal')) if parsed.get('subtotal') not in (None, '') else '',
        'vat_amount': float(parsed.get('tax')) if parsed.get('tax') not in (None, '') else '',
        'vat_percent': float(parsed.get('tax_rate')) if parsed.get('tax_rate') not in (None, '') else '',
        'discount': '',
        'grand_total': float(parsed.get('total')) if parsed.get('total') not in (None, '') else ''
    }
